...
int main(int argc, string argv[])
{
    ...
    else
    {
        ...
        if (n == 26) // Now have to check if all characters exist only once, if not then break? I will define the whole alphabet as a string maybe I will use that
            ...
            //Couldnt sleep :p Ok when I checked the walkthroughs, I didnt understand how people solve it, so I read a billions code. But I really dont understand how that code worked, so my own idea is sortstring and then stringcasecompare. 
            //Sortstring seemed like a very neat solution but it is not working in cs50 libraries? So I watched a billion more things on how to check duplicity. and now I think I understand how it is working
            // for every letter in the key (variable is j) check if that letter occures one more time in the rest of the string (lets say k) check toupper(arg[1](j)) == toupper(arg[1][k])
        {
            for (int j = 0; j < n ; j++) // checking each element of string
            {
                if (!isalpha(argv[1][j])) // if arg is alphabetic, check the lengt, has to be 26 letters
                    //Failed at "handles invalid characters in key" :/ if numeric value is on the beginning, it reprompts user, but it doesnt to that when number is at the end. so I moved it from line 26 to here
                {
                    printf("Usage: ./substitution key\n");;
                    return 1;
                }
            ...
            for (int i = 0; i < pL; i++)
            {
                if (isupper(plaintext[i])) //change every upper character with upper case of argv1
                {
...
// ok now the "supposedly" errory keys give error, and correct key continues with plaintext. I have to find a way to replace plaintext with corresponding letter
}